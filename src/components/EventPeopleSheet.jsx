    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 10350, background: "#080808", animation: "tabEnter 0.3s cubic-bezier(0.16,1,0.3,1)" }}>
      <div style={{ padding: "calc(50px + env(safe-area-inset-top, 0px)) 20px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
          <button onClick={onClose} style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 30, padding: "8px 14px", color: "rgba(255,255,255,0.7)", fontSize: 13, cursor: "pointer", fontFamily: "ui-monospace, 'SF Mono', Menlo, Monaco, monospace", flexShrink: 0 }}>← Înapoi</button>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{title}</div>
        </div>
        {onOpenEvent && (
          <button onClick={onOpenEvent} style={{ flexShrink: 0, background: "rgba(255,51,102,0.12)", border: "1px solid rgba(255,51,102,0.3)", borderRadius: 30, padding: "8px 12px", color: "#FF3366", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif", display: "flex", alignItems: "center", gap: 2 }}>
